...
import { range, isEqual } from "./Utils"
...
export function DatePickerModal(props) {
	...
	const handleSelectDate = (selected, day, date, month, year) => {
		setState({
			...state,
			selected,
		})
	...
	return (
		...
		>
			<Stack
				...
				shadow={`0px 3px 10px ${colors.Shadow}`}
				variants={{
					closed: {
						opacity: 0,
						scale: 0.95,
			...
			>
				...
				>
					{useRange ? (
						<RangeSelectionTitle start={state.start} end={state.end} />
					) : (
				...
				>
					<Button
						...
						disabled={useRange ? !(state.start && state.end) : !state.selected}
					/>
				</Stack>
			</Stack>
		</Frame>
